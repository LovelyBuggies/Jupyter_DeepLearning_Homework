...
# sgd: decay of first order momentum of gradient
...
# para-set on this experiment
opt = parser.parse_args(["--checkpoint_interval", "1", \
    "--epoch", "0",  "--decay_epoch", "50", \
    "--n_epochs", "200", "--dataset_name", "monet2photo"])
print(opt)
...
optimizer_D_A = torch.optim.SGD(D_A.parameters(), lr=opt.lr, \
    betas=(opt.b1, opt.b2))
optimizer_D_B = torch.optim.SGD(D_B.parameters(), lr=opt.lr, \
    betas=(opt.b1, opt.b2))

# Learning rate update schedulers
lr_scheduler_G = torch.optim.lr_scheduler.LambdaLR(
    optimizer_G, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, \
        opt.decay_epoch).step
)
lr_scheduler_D_A = torch.optim.lr_scheduler.LambdaLR(
    optimizer_D_A, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, \
        opt.decay_epoch).step
)
lr_scheduler_D_B = torch.optim.lr_scheduler.LambdaLR(
    optimizer_D_B, lr_lambda=LambdaLR(opt.n_epochs, opt.epoch, \
        opt.decay_epoch).step
)
...
def sample_images(batches_done):
    ...
    real_A = make_grid(real_A, nrow=7, normalize=True)
    real_B = make_grid(real_B, nrow=7, normalize=True)
    fake_A = make_grid(fake_A, nrow=7, normalize=True)
    fake_B = make_grid(fake_B, nrow=7, normalize=True)
    # Arange images along y-axis
    image_grid = torch.cat((real_A, fake_B, real_B, fake_A), 1)
    # save_image(image_grid, "images/%s/%s.png" \
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        valid = Variable(Tensor(np.ones((real_A.size(0), \
            *D_A.output_shape))), requires_grad=False)
        fake = Variable(Tensor(np.zeros((real_A.size(0), \
            *D_A.output_shape))), requires_grad=False)
        ...
        sys.stdout.write(
            ...
            [G loss: %f, adv: %f, cycle: %f, identity: %f] ETA: %s"
            % (
                epoch,
                opt.n_epochs,
                i,
