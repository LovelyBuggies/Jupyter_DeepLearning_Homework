...
class SEBottleneck(BasicBottleneck):
    expansion = 4
...
class SEResNetBottleneck(BasicBottleneck):
    expansion = 4
...
class SEResNeXtBottleneck(BasicBottleneck):
    expansion = 4
...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction,
        ...
        if input_3x3:
            layer0_modules = [
                ('conv1', nn.Conv2d(
                    3, 64, 3, stride=2, padding=1, bias=False)),
                ...
                ('conv3', nn.Conv2d(
                    64, inplanes, 3, stride=1, padding=1,
                    bias=False)),
    ...
    def _make_layer(self, block, planes, blocks, groups, reduction,
                    stride=1, downsample_kernel_size=1,
                    downsample_padding=0):
        downsample = None
...
def se_resnet50(pretrained: bool = False):
    model = SENet(SEResNetBottleneck, [3, 4, 6, 3], groups=1,
...
def se_resnet101(pretrained: bool = False):
    model = SENet(SEResNetBottleneck, [3, 4, 23, 3], groups=1,
                  reduction=16, dropout_p=None, inplanes=64,
                  input_3x3=False, downsample_kernel_size=1,
                  downsample_padding=0, num_classes=1000)
    if pretrained:
...
def se_resnet152(pretrained: bool = False):
    model = SENet(SEResNetBottleneck, [3, 8, 36, 3], groups=1,
                  reduction=16, dropout_p=None, inplanes=64,
                  input_3x3=False, downsample_kernel_size=1,
                  downsample_padding=0, num_classes=1000)
    if pretrained:
