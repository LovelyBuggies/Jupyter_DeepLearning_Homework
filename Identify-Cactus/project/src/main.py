...
save_path = '../output'  # 如果是在 kaggle 的 kernel 上跑的话，需要将 save_path 改为 './'
...
def predict(learn: Learner, name: str):
    ...
    test.to_csv('{}submission_{}.csv'.format(
        save_path, name), index=False)
    ...
    loss_df.to_csv('{}loss_{}.csv'.format(
        save_path, name), index=False)
    ...
    train_pred_df.to_csv('{}train_pred_{}.csv'.format(
        save_path, name))
    ...
    train_index_df.to_csv('{}train_index_{}.csv'.format(
        save_path, name))
